...
class AvoidReuseTest(BitcoinTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        ...
        # listunspent should show 1 single, unused 10 BTCI output
        assert_unspent(self.nodes[1], total_count=1, total_sum=10, reused_supported=True, reused_count=0)
        # getbalances should show no used, 10 BTCI trusted
        ...
        # listunspent should show 1 single, unused 5 BTCI output
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_supported=True, reused_count=0)
        # getbalances should show no used, 5 BTCI trusted
        ...
        # listunspent should show 2 total outputs (5, 10 BTCI), one unused (5), one reused (10)
        assert_unspent(self.nodes[1], total_count=2, total_sum=15, reused_count=1, reused_sum=10)
        # getbalances should show 10 used, 5 BTCI trusted
        assert_balances(self.nodes[1], mine={"used": 10, "trusted": 5})

        self.nodes[1].sendtoaddress(address=retaddr, amount=10, avoid_reuse=False)

        # listunspent should show 1 total outputs (5 BTCI), unused
        assert_unspent(self.nodes[1], total_count=1, total_sum=5, reused_count=0)
        # getbalances should show no used, 5 BTCI trusted
        assert_balances(self.nodes[1], mine={"used": 0, "trusted": 5})

        # node 1 should now have about 5 BTCI left (for both cases)
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 BTCI to A.
        [1] spends 5 BTCI from A. (leaving roughly 5 BTCI useable)
        [0] sends 10 BTCI to A again.
        [1] tries to spend 10 BTCI (fails; dirty).
        [1] tries to spend 4 BTCI (succeeds; change address sufficient)
        '''
